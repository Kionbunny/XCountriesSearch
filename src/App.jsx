...
const App = () => {
  ...
  const fetchCountries = async () => {
    ...
    try {
      ...
      // Ensure the response data is correctly assigned
      const countriesData = response.data.map((country) => ({
        name: country.name?.common || "Unknown", // Default fallback
        flag: country.flags?.png || "", // Default fallback
        cca3: country.cca3, // Ensure unique identifier for each country
      }));
      setCountries(countriesData); // Save countries to state
    } catch (error) {
